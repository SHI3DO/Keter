        ...
        if os.path.isfile(userlib + str(ctx.author.id) + ".xlsx"):
            ...
            if int(money) > val:
                ...
                if discrim < 0.02:
                    ...
                    ws.cell(row=3, column=3).value = "0"
                    embed = discord.Embed(title="도박", description= "<" + ctx.author.id + "> " + "축하합니다! 도박에 성공하셔서 3배를 획득 하셨어요! 🎉\n획득량:" + str(3*val) + " <:ket:753449741186105375>", color=0xeff0f1)
                    ...
                    ws.cell(row=3, column=3).value = "0"
                    embed = discord.Embed(title="도박", description= "<" + ctx.author.id + "> " + "축하합니다! 도박에 성공하셔서 2배를 획득 하셨어요! 🎉\n획득량:" + str(2*val) + " <:ket:753449741186105375>", color=0xeff0f1)
                else:
